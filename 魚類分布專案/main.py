...
class Window(tk.Tk):
    def __init__(self):
        ...
        # 建立Label pack是由上而下!
        title = tk.Label(
            self, text="台北市垃圾車停靠讚", bg="#345678"
        )  # 類似CSS padding的設定 上下左右各推30
        ...
        self.search_bar_button = tk.Button(
            master=searchFrame, width=8, text="搜尋", command=self.MapSearch
        )
        self.search_bar_button.grid(row=0, column=1, pady=10, padx=10)

        self.search_bar_clear = tk.Button(
            master=searchFrame, width=8, text="清除", command=self.MapClear
        )
        self.search_bar_clear.grid(row=0, column=2, pady=10, padx=10)

        self.map_widget = tkintermapview.TkinterMapView(
            MapFrame, width=800, height=600, corner_radius=0
        )
